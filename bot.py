...
DAILY_CONFIRMED_WORKSHEET_TITLE = "Сегодняшние заказы"
LEGACY_DAILY_WORKSHEET_TITLES = ["Подтвержденные заказы"]
...
ORDER_CLOSED_TEXT = (
    "К сожалению, пункт выдачи заказов сейчас не принимает заказы "
    "(принимает с 0:10 до 16:00, воскресенье выходной).\n"
    "Пожалуйста сделайте заказ в рабочее время.\n\n"
    "❗️ ВАЖНО: когда будете оформлять заказ в следующий раз, обновите QR-код в приложении, "
    "чтобы он был сегодняшним!"
)
...
def normalize_orders_data(data):
    ...
    for order_data in data.values():
        ...
        if "today_queue_active" not in order_data:
            order_data["today_queue_active"] = True
            changed = True

    return data, changed


def is_order_submission_open(user_id):
    if is_admin(user_id):
        return True
...
def get_taken_order_codes(orders, admin_id):
    codes = [
        ...
        if data.get("status") == STATUS_ACCEPTED_BY_ADMIN and data.get("today_queue_active", True)
    ]
...
def build_taken_orders_keyboard(order_code, index, total):
    ...
    kb.add(types.InlineKeyboardButton("⏸️ Деактивировать заказ", callback_data=f"accepted_deactivate|{order_code}|{index}"))
...
def send_order_for_admin_review(order_code, order_data):
    return sync_admin_review_messages(
        order_code,
        order_data,
        "📦 Новый заказ от пользователя. Он уже добавлен в сегодняшние заказы."
    )
...
def get_google_daily_orders_worksheet():
    ...
    try:
        ...
        try:
            ...
            worksheet = None
            for legacy_title in LEGACY_DAILY_WORKSHEET_TITLES:
                try:
                    worksheet = spreadsheet.worksheet(legacy_title)
                    try:
                        worksheet.update_title(DAILY_CONFIRMED_WORKSHEET_TITLE)
                    except:
                        pass
                    break
                except gspread.WorksheetNotFound:
...
def append_confirmed_order_to_daily_sheet(order_code, order_data, admin_id):
    ...
    try:
        ...
        print(f"Ошибка записи сегодняшнего заказа в Google Sheets: {e}")
...
def deactivate_order_and_notify_user(order_code, order_data):
    try:
        bot.send_message(
            order_data['user_id'],
            f"⚠️ Ваш заказ <code>{order_code}</code> деактивирован.\n"
            f"Сейчас он скрыт из сегодняшних заказов, но не удален из системы.",
            parse_mode="HTML"
        )
    except:
        pass
...
def handle_qr(message):
    ...
    if not is_order_submission_open(message.chat.id):
        bot.send_message(message.chat.id, ORDER_CLOSED_TEXT)
...
def handle_admin_review_action(call):
    ...
    if approved:
        ...
        replace_message_content(
            ...
            "✅ Подтверждение больше не требуется. Заказ уже находится в сегодняшних заказах."
        )
        return
...
def handle_taken_orders_action(call):
    ...
    if call.data.startswith("accepted_deactivate|") or call.data.startswith("accepted_delete|"):
        _, code, index = call.data.split("|")
        index = int(index)
        orders = load_data(ORDERS_FILE)
        if code in orders and orders[code].get("status") == STATUS_ACCEPTED_BY_ADMIN:
            order_data = orders[code]
            order_data["today_queue_active"] = False
            log_callback_event(call, f"Деактивирован сегодняшний заказ {code}")
            save_data(ORDERS_FILE, orders)
            deactivate_order_and_notify_user(code, order_data)
...
def handle_order_confirmation_callback(call):
    ...
    if not is_order_submission_open(user_id):
        if user_id in temp_orders:
            del temp_orders[user_id]
        log_callback_event(call, "Подтверждение заказа отклонено по времени работы ПВЗ")
        replace_message_content(
            user_id,
            call.message.message_id,
            "❌ Сейчас ПВЗ не работает, поэтому заказ не был создан.\n"
            "Пожалуйста, оформите его заново в рабочее время."
        )
        return
    ...
    replace_message_content(
        ...
        f"Он уже передан администраторам и добавлен в сегодняшние заказы. "
